...
//Referance: http://www.lunametrics.com/blog/2012/10/22/automatically-track-youtube-videos-events-google-analytics/#sr=g&m=o&cp=or&ct=-tmc&st=(opu%20qspwjefe)&ts=1416215021
...
var playFlagArray = new Array();

var isTracked = false;
var isPlayed = false, playInterval;
    ...
    for (j = 0; j < videoArray.length; j++) {
        ...
        if (videoArray[j] == videoID) {
            ...
            if (thisVideoTitle.length > 0) {
                ...
                if (showTitle == 3) {
                    thisVideoTitle = thisVideoTitle + " || " + videoID + " || " + path;
                } else if (showTitle == 2) {
                    thisVideoTitle = videoID + " || " + path;
                }
            } else {
            ...
            if (event.data == YT.PlayerState.ENDED) {
                _gaq.push(['_trackEvent', 'Videos', 'Watch to End', thisVideoTitle, parseInt(event.target.getCurrentTime())]);
                
                playFlagArray[j] = false;
            ...
            if (event.data == YT.PlayerState.PAUSED && pauseFlagArray[j] != true) {
                if (event.target.getCurrentTime() != event.target.getDuration()) {
                    _gaq.push(['_trackEvent', 'Videos', 'Pause', thisVideoTitle, parseInt(event.target.getCurrentTime())]);
...
$(document).ready(function () {
    $(document).on('click', '#VideoClose', function () {
        $.each(playFlagArray, function (index) {
            playFlagArray[index] = false;
        });
        trackYoutubeVideo();
...
function trackYoutubeVideo() {
    $.each(playerArray, function (index) {

        if (playerArray[index].A.playerState == 1) {
            //playerArray[index].stopVideo(); Stop it!!
            ...
            console.log(playerArray[index].A.videoData.title + " || " + playerArray[index].A.videoData.video_id + " || " + path + ' - ' + playerArray[index].getCurrentTime());
            var vidTitle = playerArray[index].A.videoData.title + " || " + playerArray[index].A.videoData.video_id + " || " + path;
            var time = playerArray[index].getCurrentTime();
            _gaq.push(['_trackEvent', 'Videos', 'Watch to End', vidTitle, parseInt(time)]);
